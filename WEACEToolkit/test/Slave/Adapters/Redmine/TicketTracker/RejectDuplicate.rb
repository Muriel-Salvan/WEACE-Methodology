                ...
                executeSlaveAdapter(
                  ...
                  checkCallsMatch(
                    [
                      ...
                      ['query', /^insert into journals \( journalized_id, journalized_type, user_id, notes, created_on \) values \( 123, 'Issue', query, 'Another Ticket \(ID=456\) has been closed as a duplicate of this one\.', '....-..-.. ..:..:..' \)$/],
                      ['query', 'insert into issue_relations ( issue_from_id, issue_to_id, relation_type, delay ) values ( 123, 456, \'duplicates\', NULL )'],
                      ['query', /^insert into journals \( journalized_id, journalized_type, user_id, notes, created_on \) values \( 456, 'Issue', query, 'This Ticket is a duplicate of another Ticket \(ID=123\).', '....-..-.. ..:..:..' \)$/],
                      ['insert_id', 0],
                      ['query', 'select status_id from issues where id = 456'],
                      ['query', 'update issues set status_id = 6 where id = 456'],
                      ['query', 'insert into journal_details ( journal_id, property, prop_key, old_value, value ) values ( 0, \'attr\', \'status_id\', query, 6 )']
                    ],
                    lCalls
                  )
                end
