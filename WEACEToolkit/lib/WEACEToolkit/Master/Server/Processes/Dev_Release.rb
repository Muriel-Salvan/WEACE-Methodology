      ...
      module Processes
        ...
        class Dev_Release
          ...
          def generateDeliverables(iRepositoryDir, iDeliverablesDir)
            ...
            @ContextVars = {
              'DeliverablesDir' => "\"#{iDeliverablesDir}\""
            }
            changeDir(iRepositoryDir) do
              lRealDeliverCmd = replaceVars(@DeliverCmd)
              ...
              if (lErrorCode == 0)
                ...
                Dir.glob("#{iDeliverablesDir}/ReleaseNotes") do |iReleaseNoteFileName|
                  if (File.directory?(iReleaseNoteFileName))
                    lLstErrors << "#{iReleaseNoteFileName} should be a file to be delivered. No directory admitted in #{iDeliverablesDir}/ReleaseNotes directory."
                  else
