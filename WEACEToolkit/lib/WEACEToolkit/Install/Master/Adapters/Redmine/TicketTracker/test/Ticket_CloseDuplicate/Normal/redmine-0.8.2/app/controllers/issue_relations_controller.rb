  ...
  def new
    ...
    if (request.post?)
      if (@relation.relation_type == IssueRelation::TYPE_DUPLICATES)
        ...
        lCommand = "cd %{WEACEToolkitDir}/Master/Server; /usr/bin/ruby -w WEACEMasterServer.rb Scripts_Validator Ticket_CloseDuplicate #{@relation.issue_to_id} #{@issue.id} 2>&1"
        lOutput = `#{lCommand}`
        lErrorCode = $?
        if (lErrorCode != 0)
          @relation.save
